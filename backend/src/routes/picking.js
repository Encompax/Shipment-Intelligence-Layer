...
exports.registerPickingRoutes = registerPickingRoutes;
// backend/src/routes/picking.ts
const express_1 = require("express");
const prisma_1 = require("../lib/prisma");
function registerPickingRoutes(app) {
    const router = (0, express_1.Router)();
    // GET /api/picking/tickets
    router.get("/tickets", async (req, res) => {
        try {
            const filterStatus = req.query.status;
            const filterOperator = req.query.operator;
            const where = {};
            if (filterStatus)
                where.status = filterStatus;
            if (filterOperator)
                where.assigned_to = filterOperator;
            const tickets = await prisma_1.prisma.pickingTicket.findMany({ where, orderBy: { assigned_time: "desc" } });
            res.json({
    ...
    router.get("/tickets/:ticket_id", async (req, res) => {
        try {
            ...
            const ticket = await prisma_1.prisma.pickingTicket.findUnique({ where: { picking_ticket_id: ticket_id } });
            if (!ticket) {
    ...
    router.post("/tickets/:ticket_id/scan", async (req, res) => {
        try {
            const { ticket_id } = req.params;
            ...
            const updated = await prisma_1.prisma.pickingTicket.update({
