...
exports.registerInventoryMovementRoutes = registerInventoryMovementRoutes;
...
function registerInventoryMovementRoutes(app) {
    const router = (0, express_1.Router)();
    // GET /api/inventory/movements
    router.get("/movements", async (req, res) => {
        try {
            const filterLocation = req.query.location;
            const filterItem = req.query.item;
            const filterLot = req.query.lot;
            const where = {};
            if (filterLocation) {
                where.OR = [{ from_location: filterLocation }, { to_location: filterLocation }];
            }
            if (filterItem)
                where.item_number = { contains: filterItem.toUpperCase() };
            if (filterLot)
                where.lot_number = { contains: filterLot.toUpperCase() };
    ...
    router.get("/current-positions", async (req, res) => {
        try {
            const movements = await prisma_1.prisma.inventoryMovement.findMany({ orderBy: { moved_timestamp: "asc" } });
    ...
    router.get("/item-tracking/:item_number/:lot_number", async (req, res) => {
        try {
            const { item_number, lot_number } = req.params;
            const itemMovements = await prisma_1.prisma.inventoryMovement.findMany({
    ...
    router.post("/movements", async (req, res) => {
        try {
            const { item_number, item_description, lot_number, quantity_moved, from_location, to_location, from_bin, to_bin, moved_by, reason, unit_of_measure, notes, } = req.body;
            ...
            const movement = await prisma_1.prisma.inventoryMovement.create({
                data: {
                    item_number,
