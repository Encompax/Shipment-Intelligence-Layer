...
interface DepartmentTransactionsResponse {
  department: string;
...
router.get("/departments", (req, res) => {
  ...
  const departments: Record<string, Set<number>> = {};
  ...
  const result: DepartmentTransactionsResponse[] = Object.entries(departments).map(([dept, weeks]) => ({
    department: dept,
...
router.get("/:department/:week", (req, res) => {
  ...
  const result: WeekTransactionsResponse = {
...
export const registerCycleCountRoutes = (app: any) => {
